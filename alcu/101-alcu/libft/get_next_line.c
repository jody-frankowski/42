...
**
** Because read_up_to (and read for that matter) can't know if eof is hit
** without trying to read some more, the file buffer size can be at least
** BUFF_SIZE too long.
** Because read won't tell if eof is hit without trying to read some more,
** the file buffer size can be BUFF_SIZE too long.
**
...
int				get_next_line(const int fd, char **line)
{
	...
	{
		if (!(f->buf = ft_realloc(f->buf, f->bufsz, f->bufsz + BUFF_SIZE)) ||
